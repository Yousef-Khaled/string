...
void mstring::sortwords(mstring arr[], int arrSize) {
	mstring value;
	int hole = -1;
	for (int i = 0; i < arrSize; i++) {
		value = arr[i];
		...
		while (hole > 0 && arr[hole - 1] > value) {
			arr[hole].swap(arr[hole - 1]);
			hole--;
		}
	}
}

void mstring::align(Choice c) {
...
mstring& mstring::replace(const mstring& oldWord, const mstring& newWord) {
	int temp = oldWord.size(), position;
	while (1) {
		position = this->find(oldWord, 0);
		if (position == -1)
			break;
		this->replace(position, temp, newWord);
		siz -= oldWord.size();
		siz += newWord.size();
	}
	return *this;
